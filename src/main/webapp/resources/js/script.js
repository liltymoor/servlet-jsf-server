...
function drawCanvas(r) {
    ...
    canvas.onclick = async (event) => {
        ...
        const result = await sendDot(x / R, y / R);
        console.log(result);
        if (result === null) return;
        drawDot(
            ctx,
            {x: graphDocumentX, y: graphDocumentY},
            result === "true" ? "green" : "red");
    };
}

function drawDot(ctx, dot, color='red') {
    console.log(color);
    if (dot.x < 0) dot.x = 0;
    if (dot.y < 0) dot.y = 0;
...
function drawShape(ctx, R) {
    ...
    //Прямоугольник
    ctx.fillRect(xCenter, yCenter, R, R);


    // Прямоугольный треугольник
    ctx.beginPath()
    ctx.moveTo(xCenter, yCenter);
    ctx.lineTo(xCenter - R/2, yCenter);
    ...
    ctx.beginPath();
    ctx.moveTo(xCenter, yCenter);
    ctx.arc(xCenter, yCenter, R / 2, 0, Math.PI, false);
    ctx.closePath();
    ctx.fill();
...
function initPage(r) {
    ...
    // $('.x-radio tbody tr td').on('click', function () {
    //     $('.x-radio tbody tr td').removeClass('active');
    //     $(this).addClass('active');
    // });
    //
    // $('.r-radio tbody tr td').on('click', function () {
    //     $('.r-radio tbody tr td').removeClass('active');
    //     $(this).addClass('active');
    // });
